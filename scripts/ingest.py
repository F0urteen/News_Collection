...
def _api(method: str, path: str, token: str, data: dict = None) -> dict:
    ...
    try:
        ...
        print(f"[API HTTP错误] {method} {path}: {e.code} {e.reason} | body: {err_body[:200]}")
        return {"code": e.code, "msg": err_body[:200]}
    except Exception as e:
        print(f"[API异常] {method} {path}: {e}")
...
def _api_records_all(token: str) -> list:
    ...
    while True:
        ...
        path = (f"/bitable/v1/apps/{config.BASE_TOKEN}/tables/{config.TABLE_ID}"
                f"/records?{query_str}")
...
def query_today_records(token: str) -> list[dict]:
    ...
    print(f"[查询] 今日已有 {len(today_records)} 条（总 {len(all_records)} 条）")
...
def batch_ingest(token: str, items: list[dict]) -> tuple[int, int]:
    ...
    for i in range(0, len(items), 10):
        ...
        if resp.get("code") == 0:
            ...
            success += cnt
            print(f"[入库] 第 {i//10+1} 批成功 {cnt} 条")
        else:
            print(f"[入库] 批量失败({resp.get('msg')})，逐条写入...")
            for item in batch:
